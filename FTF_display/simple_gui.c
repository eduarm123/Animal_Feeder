...
void LCD_ShowChar(uint16_t x,uint16_t y,uint16_t bcolor,uint16_t fcolor,uint8_t ch,uint8_t size,uint8_t mode)
{
	...
	if (size == 16)
    {
        LCD_SetWindows(x, y, x + size / 2 - 1, y + size - 1);//(x,y,x+8-1,y+16-1)
            /* fast show char */
            for (pos = 0; pos < size * (size / 2) / 8; pos++)
            {
                temp = asc2_1608[(uint16_t)ch * size * (size / 2) / 8 + pos];
                for (t = 0; t < 8; t++)
                {
                    if (temp & 0x80)LCD_WR_DATA16(fcolor);
	...
	if (size == 24)
	{
		LCD_SetWindows(x, y, x + size / 2 - 1, y + size - 1);
			...
			{
				temp = asc2_2412[(uint16_t)ch * (size * 16) / 8 + pos];
				if (pos % 2 == 0)
				{
					num = 8;
				}
	...
	if (size == 32)
	{
		LCD_SetWindows(x, y, x + size / 2 - 1, y + size - 1);//(x,y,x+8-1,y+16-1)
			...
			for (pos = 0; pos < size * (size / 2) / 8; pos++)
			{
				temp = asc2_3216[(uint16_t)ch * size * (size / 2) / 8 + pos];
				for (t = 0; t < 8; t++)
				{
					if (temp & 0x80)LCD_WR_DATA16(fcolor);
					else LCD_WR_DATA16(bcolor);
